...
class PriorPartyBannerAdmin
{
    ...
    /**
     * Set the default value of the prior_party_banner field to 0 on pages, notes-from-amy and notes-from-antonia.
     *
    ...
    public function filterValueByPostType(bool|null $value, int $post_id): null|bool
    {
        ...
        if (!in_array($post_type, ['page', 'note-from-amy', 'note-from-antonia']) || $value === false) {
            return $value;
        }
    ...
    public function page(): void
    {
        ...
        if ($this->banner_reference) {
            ...
            if ($this->review_tracked_events) {
                $events = $this->getPopulatedTrackEvents(
                    null,
            ...
            if (!empty($this->posts)) {
                ...
                foreach ($this->posts as $post) {
                    ...
                    // latest event
                    $event_data = $this->getLatestEventDisplayString($post->ID);
                    ...
                    if ($this->review_tracked_events) {
                        ...
                        $readable_events = isset($events[$post->ID]) ? array_map(
                            [$this, 'populatedEventToReadableFormat'],
                            $events[$post->ID]
                        ) : [];
