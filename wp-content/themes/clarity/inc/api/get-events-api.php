...
function get_events_api( $taxonomy, $tax_id = false ) {
	...
	*/
	...
	if ( $taxonomy === 'search' ) {
		...
		$response = wp_remote_get( $siteurl . '/wp-json/intranet/v2/region-events/' . $agency_name . '/' . $tax_id . '/' . '30' );
	} elseif ( $taxonomy === 'region_landing' ) {
		$response = wp_remote_get( $siteurl . '/wp-json/intranet/v2/region-events/' . $agency_name . '/' . $tax_id . '/' . '4' );
