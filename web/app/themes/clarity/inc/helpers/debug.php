...
class Debug
{
   ...
   */
    ...
    private static function get_array($var, $level)
    {
        foreach ($var as $x1 => $x2) {
            ...
            if (is_array($x2)) {
                ...
                if (is_bool($x2)) {
                    $t = 'boolean';
                    $x2 = '<span class="__g">'.(($x2) ? 'true' : 'false').'</span>';
                } elseif (is_integer($x2)) {
                    $t = 'int';
                    $x2 = '<span class="__r">'.$x2.'</span>';
                } elseif (is_float($x2)) {
                    $t = 'float';
                    $x2 = '<span class="__r">'.$x2.'</span>';
                } elseif (is_string($x2)) {
                    $t = 'string';
                    $x2 = '<span class="__r">[</span><span class="__b">'.htmlspecialchars($x2).'</span><span class="__r">]</span>';
                } elseif (is_null($x2)) {
                    $t = 'null';
                    $x2 = '<span class="__g">NULL</span>';
                } elseif (is_object($x2)) {
                    $t = 'object';
                    $x2 = '<span class="__g">Object</span>';
                } else {
                    $t = gettype($x2);
                    $x2 = '???';
                }
    ...
    private static function get_var($var, $level)
    {
        if (is_bool($var)) {
            $t = 'boolean';
            $var = '<span class="__g">'.(($var) ? 'true' : 'false').'</span>';
        } elseif (is_integer($var)) {
            $t = 'int';
            $var = '<span class="__r">'.$var.'</span>';
        } elseif (is_float($var)) {
            $t = 'float';
            $var = '<span class="__r">'.$var.'</span>';
        } elseif (is_string($var)) {
            $t = 'string';
            $var = '<span class="__r">[</span><span class="__b">'.htmlspecialchars($var).'</span><span class="__r">]</span>';
        } elseif (is_null($var)) {
            $t = 'null';
            $var = '<span class="__g">NULL</span>';
